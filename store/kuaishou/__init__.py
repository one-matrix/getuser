...
async def update_ks_video_comment(video_id: str, comment_item: Dict):
    ...
    tid = task_id_var.get() if hasattr(task_id_var, 'get') else ""
    if tid:
        save_comment_item["task_id"] = tid
    utils.logger.info(
...
async def save_creator(user_id: str, creator: Dict):
    ...
    local_db_item = {
        ...
        'ip_location': "",
        'follows': str(ownerCount.get("follow", 0)),
        'fans': str(ownerCount.get("fan", 0)),
        'interaction': str(ownerCount.get("photo_public", 0)),
        "last_modify_ts": utils.get_current_timestamp(),
    }
