...
async def save_dm_record(
    ...
    utils.logger.info(
        f"[store.douyin.save_dm_record] DM to {nickname}({sec_uid}): "
        f"{'success' if success else 'failed'}, error={error}"
    )
    await DouyinStoreFactory.create_store().store_dm_record(dm_item=save_item)
