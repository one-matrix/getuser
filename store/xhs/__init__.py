...
async def update_xhs_note(note_item: Dict):
    ...
    local_db_item = {
        ...
        "liked_count": str(interact_info.get("liked_count", 0)),  # Like count
        "collected_count": str(interact_info.get("collected_count", 0)),  # Collection count
        "comment_count": str(interact_info.get("comment_count", 0)),  # Comment count
        "share_count": str(interact_info.get("share_count", 0)),  # Share count
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    local_db_item = {
        ...
        "sub_comment_count": str(comment_item.get("sub_comment_count", 0)),  # Sub-comment count
        "pictures": ",".join(comment_pictures),  # Comment pictures
        "parent_comment_id": str(target_comment.get("id", 0)),  # Parent comment ID
        "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp (Generated by MediaCrawler, mainly used to record the latest update time of a record in DB storage)
...
async def save_creator(user_id: str, creator: Dict):
    ...
    local_db_item = {
        ...
        'follows': str(follows),  # Following count
        'fans': str(fans),  # Fans count
        'interaction': str(interaction),  # Interaction count
