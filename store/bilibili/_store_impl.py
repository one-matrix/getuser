    ...
    async def store_content(self, content_item: Dict):
        ...
        async with get_session() as session:
            ...
            if not video_detail:
                ...
                # 过滤掉模型中不存在的字段(如 video_type, title, desc 等)
                valid_cols = {c.name for c in BilibiliVideo.__table__.columns}
                filtered_item = {k: v for k, v in content_item.items() if k in valid_cols}
