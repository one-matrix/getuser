    ...
    async def request(self, method, url, **kwargs) -> Any:
        ...
        if self._account_pool:
            ...
            if account and account.cookie:
                ...
                current_alias = account.cookie_alias or "bili账号"
                cookie_preview = account.cookie[:30] + "..." if len(account.cookie) > 30 else account.cookie
                utils.logger.info(
                    f"[BilibiliClient.request] >>> {current_alias} | "
            ...
            if self._account_pool and current_account_id:
                ...
                if should_switch:
                    utils.logger.warning(
                        f"[BilibiliClient.request] Network error ({fail_type}), switching account..."
                    )
        ...
        # 风控检测
        from api.services.account_pool_mixin import detect_bili_risk
        fail_type = detect_bili_risk(data, response.text)
        if fail_type:
            if self._account_pool and current_account_id:
                should_switch = await self._account_pool.report_failure(current_account_id, fail_type)
                if should_switch:
                    utils.logger.warning(
                        f"[BilibiliClient.request] Risk detected ({fail_type}), switching account..."
                    )
                    await self._account_pool.switch_account(fail_type)
        ...
        if data.get("code") != 0:
            ...
            if self._account_pool and current_account_id:
                ...
                utils.logger.info(f"[BilibiliClient.request] ✓ 成功 | {current_alias}")
