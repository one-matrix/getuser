    ...
    async def request(self, method, url, **kwargs) -> Union[Response, Dict]:
        ...
        if self._account_pool:
            ...
            if account and account.cookie:
                ...
                current_alias = account.cookie_alias or "wb账号"
                cookie_preview = account.cookie[:30] + "..." if len(account.cookie) > 30 else account.cookie
                utils.logger.info(
                    f"[WeiboClient.request] >>> {current_alias} | "
            ...
            if self._account_pool and current_account_id:
                ...
                if should_switch:
                    utils.logger.warning(
                        f"[WeiboClient.request] Network error ({fail_type}), switching account..."
                    )
        ...
        # 风控检测
        from api.services.account_pool_mixin import detect_wb_risk
        fail_type = detect_wb_risk(data, response.text)
        if fail_type:
            if self._account_pool and current_account_id:
                should_switch = await self._account_pool.report_failure(current_account_id, fail_type)
                if should_switch:
                    utils.logger.warning(
                        f"[WeiboClient.request] Risk detected ({fail_type}), switching account..."
                    )
                    await self._account_pool.switch_account(fail_type)
        ...
        if ok_code == 0:
            ...
            if self._account_pool and current_account_id:
                await self._account_pool.report_success(current_account_id)
                utils.logger.info(f"[WeiboClient.request] ✓ 成功 | {current_alias}")
