...
class WeiboCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        try:
            ...
            self._account_pool = await init_platform_account_pool("wb", logger=utils.logger)
        except Exception as e:
            utils.logger.warning(f"[WeiboCrawler.start] init account_pool failed (non-fatal): {e}")
    ...
    async def create_weibo_client(self, httpx_proxy: Optional[str]) -> WeiboClient:
        ...
        weibo_client_obj = WeiboClient(
            ...
            proxy_ip_pool=self.ip_proxy_pool,  # Pass proxy pool for automatic proxy refresh
