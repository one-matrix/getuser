...
async def get_a_bogus(url: str, params: str, post_data: dict, user_agent: str, page: Page = None):
    ...
    优先使用浏览器动态生成签名（更准确），失败时回退到静态 JS
    """
    # 如果有 page 对象，优先使用浏览器动态生成签名
    if page:
        try:
            a_bogus = await get_a_bogus_from_playwright(params, post_data, user_agent, page)
            if a_bogus and len(a_bogus) > 10:
                return a_bogus
        except Exception as e:
            # 抖音反爬签名常变化,Playwright 动态签名失败属预期,JS 回退可正常工作
            import logging
            logging.getLogger(__name__).debug(f"[get_a_bogus] Playwright sign failed, fallback to JS: {e}")
...
async def get_a_bogus_from_playwright(params: str, post_data: dict, user_agent: str, page: Page):
    ...
    策略：
    1. 优先通过浏览器内 XHR 请求让前端自动签名，拦截获取 a_bogus
    2. 回退到 frontierSign 获取 X-Bogus
    3. 回退到 bdms.init
    """
    if not post_data:
        post_data = ""
    
    # 方法1: 通过浏览器内 XHR 请求让前端自动签名（最可靠）
    try:
        a_bogus = await _get_a_bogus_via_browser_xhr(page, params)
        if a_bogus and len(str(a_bogus)) > 10:
            import logging
            logging.getLogger(__name__).info("[get_a_bogus_from_playwright] Browser XHR auto-sign succeeded")
            return a_bogus
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"[get_a_bogus_from_playwright] Browser XHR method failed: {e}")
    
    # 方法2: 使用 byted_acrawler.frontierSign 获取 X-Bogus
    has_acrawler = await page.evaluate("() => typeof window.byted_acrawler !== 'undefined' && window.byted_acrawler.frontierSign")
    if has_acrawler:
        try:
            sign_result = await page.evaluate(
                "([params]) => { const sign = window.byted_acrawler.frontierSign({ url: params, method: 'GET' }); return sign; }",
                [params]
            )
            if sign_result and isinstance(sign_result, dict):
                # frontierSign 可能返回 X-Bogus 或 a_bogus
                a_bogus = sign_result.get("a_bogus") or sign_result.get("X-Bogus")
                if a_bogus and len(str(a_bogus)) > 5:
                    import logging
                    logging.getLogger(__name__).info(f"[get_a_bogus_from_playwright] frontierSign returned: {list(sign_result.keys())}")
                    return a_bogus
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"[get_a_bogus_from_playwright] frontierSign failed: {e}")
    
    # 方法3: 尝试 bdms.init
    has_bdms = await page.evaluate("() => typeof window.bdms !== 'undefined' && window.bdms && window.bdms.init")
    if not has_bdms:
        import logging
        logging.getLogger(__name__).debug("[get_a_bogus_from_playwright] window.bdms not found, injecting bdms.js...")
        try:
            await page.add_script_tag(url="https://lf-c-flwb.bytetos.com/obj/rc-client-security/web/stable/1.0.1.18/bdms.js")
            try:
                await page.wait_for_function(
                    "() => typeof window.bdms !== 'undefined' && window.bdms && window.bdms.init",
                    timeout=10000
                )
                has_bdms = True
            except Exception:
            ...
            import logging
            logging.getLogger(__name__).warning(f"[get_a_bogus_from_playwright] Failed to load bdms: {e}")
    
    if has_bdms:
        try:
            a_bogus = await page.evaluate(
                "([params, post_data, ua]) => { try { return window.bdms.init._v[2].p[42].apply(null, [0, 1, 8, params, post_data, ua]); } catch(e) { return null; } }",
    ...
    raise Exception("All a_bogus generation methods failed")


async def _get_a_bogus_via_browser_xhr(page: Page, params: str):
    """
    通过在浏览器内发起 XHR 请求，让抖音前端自动添加签名参数，
    拦截请求获取 a_bogus。这是最可靠的方式，因为签名由前端自动完成。
    """
    a_bogus = await page.evaluate("""
        async ([params]) => {
            return new Promise((resolve) => {
                ...
                const baseUrl = '/aweme/v1/web/general/search/single/?' + params;
                ...
                xhr.onload = function() {
                    ...
                    const match = finalUrl.match(/a_bogus=([^&]+)/);
                    resolve(match ? decodeURIComponent(match[1]) : null);
                };
