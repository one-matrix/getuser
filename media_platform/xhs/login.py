...
class XiaoHongShuLogin(AbstractLogin):
    ...
    def _parse_netscape_cookies(self, cookie_str: str) -> List[Dict]:
        """Parse Netscape format cookies from .env file.
        Format: name\tvalue\tdomain\tpath\texpires\tsize\thttpOnly\tsecure\tsameSite\tpriority
        """
        cookies = []
        ...
        for line in cookie_str.strip().split("\n"):
            ...
            name = parts[0]
            value = parts[1]
            cookie_domain = parts[2] if parts[2] else domain
            path = parts[3] if parts[3] else "/"
            expires_str = parts[4] if len(parts) > 4 else ""
            cookie_dict = {
                "name": name,
                "value": value,
            ...
            if expires_str and expires_str not in ("会话", "Session"):
                try:
                    expires_dt = datetime.fromisoformat(expires_str.replace("Z", "+00:00"))
                    cookie_dict["expires"] = int(expires_dt.timestamp())
                except Exception:
                    pass
    ...
    async def login_by_cookies(self):
        ...
        # Try to load full Netscape format cookies from .env
        xhs_cookies_env = os.getenv("XHS_COOKIES", "")
        if xhs_cookies_env and "\t" in xhs_cookies_env:
            cookies = self._parse_netscape_cookies(xhs_cookies_env)
            if cookies:
                utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Loading {len(cookies)} cookies from XHS_COOKIES env")
                await self.browser_context.add_cookies(cookies)
                return
        # Fallback to simple key=value cookie string
        cookie_dict = utils.convert_str_cookie_to_dict(self.cookie_str)
        if not cookie_dict:
            utils.logger.warning("[XiaoHongShuLogin.login_by_cookies] No cookies found, login may fail")
            return
