...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def __init__(
        ...
        *,
        ...
        account_pool=None,  # 多账号池（可选，集成Cookie+IP自动切换）
    ):
        self.proxy = proxy
    ...
    @retry(stop=stop_after_attempt(5), wait=wait_fixed(2), retry=retry_if_not_exception_type(NoteNotFoundError))
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        """
        Wrapper for httpx common request method, processes request response
        带账号池自动切换：检测到风控时自动切换Cookie+IP组合
        """
        import random
        ...
        if self._account_pool:
            ...
            if account and account.cookie:
                ...
                current_alias = "默认"
                utils.logger.warning("[XiaoHongShuClient.request] No healthy account, using default cookie")
        else:
            current_account_id = None
            current_interface = None
            current_alias = "无账号池"
        ...
        try:
            ...
            if response.status_code == 471 or response.status_code == 461:
                ...
                if self._account_pool and current_account_id:
                    should_switch = await self._account_pool.report_failure(current_account_id, "captcha")
                    if should_switch:
                        utils.logger.warning(f"[XiaoHongShuClient.request] CAPTCHA, switching account...")
                        await self._account_pool.switch_account("captcha")
                sleep_time = random.uniform(15, 30)
                utils.logger.info(f"[XiaoHongShuClient.request] Sleeping {sleep_time:.1f}s before retry due to CAPTCHA...")
            ...
            # 业务码风控检测
            from api.services.account_pool_mixin import detect_xhs_risk
            fail_type = detect_xhs_risk(data, response.text)
            if fail_type:
                if self._account_pool and current_account_id:
                    should_switch = await self._account_pool.report_failure(current_account_id, fail_type)
                    if should_switch:
                        utils.logger.warning(
                            f"[XiaoHongShuClient.request] Risk detected ({fail_type}), switching account..."
                        )
            ...
            if data.get("success"):
                if self._account_pool and current_account_id:
                    await self._account_pool.report_success(current_account_id)
                    utils.logger.info(f"[XiaoHongShuClient.request] ✓ 成功 | {current_alias}")
                return data.get("data", data.get("success", {}))
            elif data.get("code") == self.IP_ERROR_CODE:
                if self._account_pool and current_account_id:
                    await self._account_pool.report_failure(current_account_id, "blocked")
                    await self._account_pool.switch_account("blocked")
        ...
        except (NoteNotFoundError, IPBlockError, DataFetchError):
            raise
            ...
            if self._account_pool and current_account_id:
                ...
                if should_switch:
                    utils.logger.warning(
                        f"[XiaoHongShuClient.request] Network error ({fail_type}), switching account..."
                    )
                    await self._account_pool.switch_account(fail_type)
            raise
